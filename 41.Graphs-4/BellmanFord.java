import java.util.*;

public class BellmanFord{

    static class Edge{
        int src,dest,wt;

        public Edge(int s,int d,int w){
            this.src=s;
            this.dest=d;
            this.wt=w;
        }
    }

    static void createGraph(ArrayList<Edge> graph[]){
        for(int i=0;i<graph.length;i++){
            graph[i]=new ArrayList<>();//initialization 
       }
       graph[0].add(new Edge(0,1,2));
       graph[0].add(new Edge(0,2,4));

       graph[1].add(new Edge(1,2,-4));

       graph[2].add(new Edge(2,3,2));

       graph[3].add(new Edge(3,4,4));

       graph[4].add(new Edge(4,1,-1));

    }

    public static void bellmanFord(ArrayList<Edge> graph[],int src){
        int dist[]=new int[graph.length];
        for(int i=0;i<dist.length;i++){
            if( i != src){
                dist[i]=Integer.MAX_VALUE;//infinity initialization
            }
        }
        int V = graph.length;
        //Algorithm

        for(int i=0;i<V-1;i++){//-> O(V)
            //edges-O(E)
            for(int j=0;j<graph.length;j++){//sare vertices nikal liya
                for(int k=0;k<graph[j].size();k++){//usse connected sare edges nikal lia
                    Edge e = graph[j].get(k);//edge ki value yaha se u,v ki value milegi
                    int u=e.src;
                    int v=e.dest;
                    int w=e.wt;
                    if(dist[u]!=Integer.MAX_VALUE && dist[u]+w<dist[v]){//dist[u]!=Integer.MAX_VALUE imp condn kyuki java me infinity me add kia toh -ve
                        dist[v]=dist[u]+w;
                    }
                }
            }
        }
        //print
        for(int i=0;i<dist.length;i++){
           
            System.out.print(dist[i]+" ");
        }
        System.out.println();

    }

  
    @SuppressWarnings("unchecked")
    public static void main(String args[]){
        int V= 5 ;
        ArrayList<Edge> graph[]= new ArrayList[V];  
        createGraph(graph);
        bellmanFord(graph, 0);
    }
   } 
